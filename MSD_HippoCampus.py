...
LossFunction = monai.losses.DiceLoss(include_background=True, to_onehot_y=False, softmax=True)

MetricDice = monai.metrics.DiceMetric(include_background=True, reduction="mean")
MetricDiceAT = monai.metrics.DiceMetric(include_background=True, reduction="mean")
MetricDicePT = monai.metrics.DiceMetric(include_background=True, reduction="mean")
...
def evaluate(epoch, test_loader):
    ...
    with torch.no_grad():
        for step, batch in enumerate(epoch_iterator):
            ...
            MetricDiceAT(bi_output[:, 1:2], y[:, 1:2])
            dice_AT = MetricDiceAT.aggregate().item()
            MetricDicePT(bi_output[:, 2:3], y[:, 2:3])
            dice_PT = MetricDicePT.aggregate().item()
            
            mean_AT += dice_AT
            mean_PT += dice_PT
            
            epoch_iterator.set_description(f"Evaluating (Epoch {epoch}/{EPOCHS}) (loss={loss.item():.5f}) (dice={dice_score:.5f})")
    ...
    MetricDiceAT.reset()
    MetricDicePT.reset()
    return mean_epoch_loss, mean_dice_score, mean_AT, mean_PT 
...
for epoch in range(1, EPOCHS+1):
    ...
    val_loss, val_dice, val_AT, val_PT = evaluate(epoch, val_loader)
    losses['train'].append(train_loss)
    losses['val'].append(val_loss)
    dice_scores['train'].append(train_dice)
    dice_scores['val'].append(val_dice)
    AT_scores['val'].append(val_AT)
    PT_scores['val'].append(val_PT)
    
    print(f"Epoch {epoch}: Val Loss: {val_loss:.6f}, Overall Dice: {val_dice:.6f}, AT: {val_AT:.6f}, PT: {val_PT:.6f}")
    ...
    if val_loss < best_metric:
        ...
        if MODEL_SAVE and epoch > (EPOCHS-100):
            model_name = f'{best_epoch}_{best_metric:.6f}.pth'
            torch.save(model.state_dict(), os.path.join(MODEL_SAVE_PATH, model_name))
            print('saved model')

mean_loss = sum(losses['val']) / len(losses['val'])
mean_overall = sum(dice_scores['val']) / len(dice_scores['val'])
mean_AT = sum(AT_scores['val']) / len(AT_scores['val'])
mean_PT = sum(PT_scores['val']) / len(PT_scores['val'])
...
TXT_SAVE_NAME = f'epoch{EPOCHS}_Hippocampus_Lite_Swin_UNETR2.txt'
...
with open(result_file_path, 'w') as result_file:
    ...
    result_file.write(f'Best AT Dice Score: {best_AT:.6f}\n')
    result_file.write(f'Best PT Dice Score: {best_PT:.6f}\n')
    result_file.write(f'Mean Loss: {mean_loss:.6f}\n')
    result_file.write(f'Mean Overall Dice Score: {mean_overall:.6f}\n')
    result_file.write(f'Mean AT Dice Score: {mean_AT:.6f}\n')
    result_file.write(f'Mean PT Dice Score: {mean_PT:.6f}\n')
...
TXT_SAVE_NAME2 = f'epoch{EPOCHS}_Hippocampus_Lite_Swin_UNETR_overlay_result2.png'
